...
function exibirAvisoLindo(titulo, mensagem) {
...
export function iniciarNovoPedido() {
    ...
    if (btAdd) {
        ...
        btAdd.addEventListener("click", async () => {
            if (!inputProduto || !quantValor || !inputProduto.value || !unidProdutoMolde || !list)
                return;
            ...
            if (produtoExistente) {
                ...
                await exibirAvisoLindo("Produto Duplicado", `O item "${produtoExistente.nome}" Produto ja adicionado na lista`);
